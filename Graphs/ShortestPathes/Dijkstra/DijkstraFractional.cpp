...
namespace matrix {
    // create template struct for edge if u wanna store additional info (sample: Graphs/Flows/MCMF/JhonsonPotentials.cpp)
    ...
    public:
        ...
        dijkstra(const vector<vector<dijkstra_edge<T>>>& g, const int LARGEST_VERTEX, const int BEGIN, const int INDEXATION = 1, const T EPS = 1e-6)
        ...
        void do_dijkstra(const vector<vector<dijkstra_edge<T>>>& g, const int LARGEST_VERTEX, const int BEGIN, const int INDEXATION) {
            ...
            while (true) {
                ...
                for (int to = INDEXATION; to < g[nearest].size(); ++to) {
                    if (used[to] || g[nearest][to].w < -EPS) continue;
    ...
    public:
        ...
        dijkstra_with_path(const vector<vector<dijkstra_edge<T>>>& g, const int LARGEST_VERTEX, const int BEGIN, const int INDEXATION = 1, const T EPS = 1e-6)
        : MAX_D(numeric_limits<T>::max() - 1)
        , EPS(EPS)
        , BEGIN(BEGIN)
        , d(LARGEST_VERTEX + 1, MAX_D)
        ...
        void do_dijkstra(const vector<vector<dijkstra_edge<T>>>& g, const int LARGEST_VERTEX, const int INDEXATION) {
            ...
            while (true) {
                ...
                for (int to = INDEXATION; to < LARGEST_VERTEX + 1; ++to) {
                    ...
                    if (auto new_d = min_d + g[nearest][to].w; d[to] > new_d) {
                        d[to] = new_d;
                        prev[to] = nearest;
                    }
                }
...
namespace list {
    ...
    private:
        void dense_dijkstra(const vector<vector<dijkstra_edge<T>>>& g, const int LARGEST_VERTEX, const int INDEXATION) {
            vector<bool> used(LARGEST_VERTEX + 1);
            d[BEGIN] = 0;
